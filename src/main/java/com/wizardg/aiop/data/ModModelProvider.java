...
public class ModModelProvider extends ModelProvider {
    ...
    public ModModelProvider(PackOutput output) {
        super(output, AIOPAllinOnePipe.MODID);
    }
    ...
    // Cable faces get tintindex 0 so colored pipes can tint them.
    private static Consumer<ElementBuilder> core() {
        return e -> e.from(6, 6, 6).to(10, 10, 10).allFaces((dir, f) -> f.texture(CABLE).tintindex(0));
    }
    ...
    // Closes an open cable end, the default uv lands on the framed centre of cable.png.
    private static Consumer<ElementBuilder> cap(Direction side) {
        return e -> e.face(side, f -> f.texture(CABLE).tintindex(0));
    }

    // EnderIO style connector plate against the neighbouring block.
    ...
    private static Identifier model(String name) {
        return Identifier.fromNamespaceAndPath(AIOPAllinOnePipe.MODID, "block/" + name);
    }
